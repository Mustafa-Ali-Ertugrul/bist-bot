...
def test_short_signal_profit_sign(params, engine_replay):
    short = _signal(
        signal_type="🔴 SAT", score=-40.0, price=100.0, stop_loss=105.0, target_price=90.0, id=15
    )
    bars = _bars(
        [
            ("2026-08-10", 100, 101, 99, 100),
...
def test_window_open_when_no_future_bars(params, engine_replay):
    ...
    recent = _signal(
        timestamp=datetime(today.year, today.month, today.day, tzinfo=UTC) - timedelta(days=1),
        id=16,
    )
    bars = _bars(
        [
