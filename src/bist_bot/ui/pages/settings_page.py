...
def render_settings_page() -> None:
    ...
    st.session_state.refresh_interval = st.select_slider(
        "Tarama araligi (dk)", options=[1, 3, 5, 10, 15], value=st.session_state.refresh_interval
    )
    ...
    st.session_state.notify_telegram = st.toggle(
        "Telegram bildirimi", value=st.session_state.notify_telegram, disabled=not tg_ready
    )
