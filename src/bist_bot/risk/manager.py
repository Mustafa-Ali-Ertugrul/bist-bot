...
class RiskManager:
    def __init__(
        ...
        self.capital = float(capital) if capital is not None else float(getattr(settings, "INITIAL_CAPITAL", 8500.0))
    ...
    def register_position(self, ticker: str, df: pd.DataFrame) -> None:
        if df is None or df.empty:
            return
        history_slice = cast(pd.DataFrame, df[[c for c in ["close", "high", "low", "atr"] if c in df.columns]].copy())
        self._portfolio_history[ticker] = history_slice

    def _restore_persisted_positions(self, data: dict) -> None:
        if self.position_repository is None or not hasattr(self.position_repository, "get_active_position_tickers"):
            return
    ...
    def _calc_atr_levels(self, df: pd.DataFrame, price: float, levels: RiskLevels) -> RiskLevels:
        return stop_helpers.calc_atr_levels(df, price, levels, self.atr_stop_mult, self.atr_target_mult)

    def _calc_support_resistance(self, df: pd.DataFrame, price: float, levels: RiskLevels) -> RiskLevels:
    ...
    def _calc_fixed_percent(self, price: float, levels: RiskLevels) -> RiskLevels:
        return stop_helpers.calc_fixed_percent(price, levels, self.fixed_stop_pct, self.fixed_target_pct)
    ...
    def _apply_position_budget(self, price: float, levels: RiskLevels) -> None:
        sizing_helpers.apply_position_budget(price, levels, self.capital, self.max_risk_pct)
    ...
    def _calculate_risk_throttle(self, atr_pct: float) -> float:
        return sizing_helpers.calculate_risk_throttle(atr_pct, self.atr_baseline_pct, self.atr_min_risk_scale)
