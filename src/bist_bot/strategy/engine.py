...
class StrategyEngine:
    ...
    def _extract_timeframes(
        ...
        if isinstance(market_data, dict):
            ...
            if trend_df is None or trigger_df is None:
                raise ValueError("Multi-timeframe veri 'trend' ve 'trigger' anahtarlar─▒n─▒ i├ºermeli")
    ...
    def analyze(
        ...
        adx = last.get("adx")
        if not pd.notna(adx):
            logger.debug(f"  {ticker}: ADX hesaplanamad─▒ (NaN) - sinyal ├╝retme")
            return None
        if adx < getattr(settings, "ADX_THRESHOLD", 20):
            logger.debug(f"  {ticker}: ADX d├╝┼ƒ├╝k ({adx:.1f}) - Trend yok, sinyal ├╝retme")
            return None

        regime = detect_regime(df)
        if regime == MarketRegime.SIDEWAYS:
            reasons.append("Piyasa rejimi yatay - skor etkisi azalt─▒ld─▒")
        ...
        if regime == MarketRegime.SIDEWAYS:
            ...
            if abs(score) < self.BUY_THRESHOLD:
                logger.debug(f"  {ticker}: Yatay piyasada skor zay─▒f ({score:.1f}) - sinyal yok")
                return None
        ...
        if signal_type in {SignalType.STRONG_BUY, SignalType.BUY, SignalType.WEAK_BUY}:
            if enforce_sector_limit and not self.risk_manager.check_sector_limit(ticker):
                logger.debug(f"  {ticker}: sekt├╢r limiti nedeniyle sinyal atland─▒")
                return None
        ...
        if signal_type in {SignalType.STRONG_BUY, SignalType.BUY, SignalType.WEAK_BUY}:
            ...
            if risk_levels.blocked_by_correlation or risk_levels.position_size <= 0:
                logger.debug(f"  {ticker}: portf├╢y riski nedeniyle sinyal atland─▒")
                return None
            if self.meta_model is not None and hasattr(self.meta_model, "predict_probability"):
                ...
                if risk_levels.position_size <= 0:
                    logger.debug(f"  {ticker}: meta-model sizing nedeniyle sinyal atland─▒")
                    return None
        ...
        signal.reasons.append(
            f"Pozisyon: {risk_levels.position_size} lot | Risk B├╝t├ºesi: Γé║{risk_levels.risk_budget_tl:.2f}"
        )
        signal.reasons.append(
        ...
        if risk_levels.liquidity_value > 0:
            signal.reasons.append(
                f"Likidite: Γé║{risk_levels.liquidity_value:,.0f} ort. islem degeri"
            )
        if risk_levels.correlated_tickers:
            signal.reasons.append(
                f"Korelasyon limiti: x{risk_levels.correlation_scale:.2f} | ─░li┼ƒkili: {', '.join(risk_levels.correlated_tickers)}"
            )

        if multi_timeframe and getattr(settings, "MTF_ENABLED", True):
            if not self._apply_confluence(signal.signal_type, trend_bias, signal.reasons):
                logger.debug(f"  {ticker}: MTF confluence nedeniyle sinyal atland─▒")
                return None
