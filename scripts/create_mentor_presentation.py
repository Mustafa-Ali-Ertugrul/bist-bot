...
def build_presentation(output_path):
    ...
    def add_header(slide, slide_num_str, title_text, subtitle_text):
        ...
        if slide_num_str:
            badge = slide.shapes.add_shape(
                MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(0.45), Inches(0.6), Inches(0.4)
            )
            badge.fill.solid()
    ...
    # Top Tag
    tag_box = s1.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(2.0), Inches(1.8), Inches(4.2), Inches(0.4)
    )
    tag_box.fill.solid()
    ...
    add_header(
        s2,
        "02",
        "PROBLEM VE HEDEF KULLANICI KİTLESİ",
        "Sorun veri eksikliği değil; karar disiplini, açıklanabilirlik ve parçalı araç problemidir.",
    )

    # Left Card: Bireysel Yatırımcı
    c1 = s2.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.4), Inches(5.6), Inches(4.3)
    )
    c1.fill.solid()
    ...
    items1 = [
        (
            "Sinyal Kirliliği:",
            " Çok sayıda indikatörü eş zamanlı takip ederken çelişkili sinyaller arasında kararsız kalır.",
        ),
        (
            "Kara Kutu Şüpheciliği:",
            " Üretilen AL/SAT alarmlarının arkasındaki mantığı ve risk nedenini göremez.",
        ),
        (
            "Disiplinsiz İşlem:",
            " Stop-loss, hedef fiyat ve pozisyon büyüklüğü planlamadan duygusal işlemler yapar.",
        ),
    ]
    ...
    c2 = s2.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.833), Inches(1.4), Inches(5.7), Inches(4.3)
    )
    c2.fill.solid()
    ...
    items2 = [
        (
            "Parçalı Araç Yükü:",
            " Tarama, grafik inceleme, risk hesabı ve bildirim için ayrı platformlar kullanmak zorundadır.",
        ),
        (
            "Tekilleştirilemeyen Testler:",
            " Strateji fikirlerini geriye dönük (backtest) ve canlı öncesi (paper trade) tutarlı doğrulamakta zorlanır.",
        ),
        (
            "Ölçeklenme Engeli:",
            " BIST evrenindeki tüm hisseleri üst zaman dilimi (MTF) uyumuyla manuel tarayamaz.",
        ),
    ]
    ...
    b_banner = s2.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(5.9), Inches(11.733), Inches(1.1)
    )
    b_banner.fill.solid()
    ...
    add_header(
        s3,
        "03",
        "OTONOM KARAR VE YÜRÜTME HATTI (PIPELINE)",
        "Her BIST hissesi 5 aşamalı standart, disiplinli ve izlenebilir bir süreçten geçer.",
    )

    steps = [
        (
            "1. VERİ AKIŞI",
            "BIST Evrensel Verisi",
            [
        ...
        (
            ...
            [
                "• 4 Kanıt Grubu Puanı",
                "• -100 ile +100 Skor",
                "• Sert Veto Filtreleri",
                "• Şeffaf Açıklama Listesi",
            ],
            ACCENT_AMBER,
        ),
        (
    ...
    for i, (num_title, sub_title, bullet_list, accent_clr) in enumerate(steps):
        ...
        # Content Box
        ct_box = s3.shapes.add_textbox(
            cx + Inches(0.1), cy + Inches(1.05), card_w - Inches(0.2), Inches(3.7)
        )
    ...
    add_header(
        s4,
        "04",
        "KARAR MEKANİZMASI: 4 TEMEL KANIT GRUBU",
        "Sinyaller tek bir göstergeye değil, 4 bağımsız teknik boyuta dayanır.",
    )

    ev_groups = [
        (
            "⚡ MOMENTUM KANITLARI",
            "Aşırı Alım / Satım & Hız",
            [
        ...
        (
            "📈 TREND & REJİM KANITLARI",
            "Yön Gücü & Zaman Dilimi",
            [
        ...
        (
            "📊 HACİM & AKIŞ KANITLARI",
            "Katılım & Hacim Doğrulaması",
            [
        ...
        (
            "🛡️ YAPI & RİSK KANITLARI",
            "Volatolite & Yapısal Seviyeler",
            [
    ...
    for i, (g_title, _g_sub, g_bullets, accent_clr) in enumerate(ev_groups):
        cx, cy = positions[i]
        ...
        # Header Box
        hdr_box = s4.shapes.add_textbox(
            cx + Inches(0.2), cy + Inches(0.15), card_w - Inches(0.4), Inches(0.5)
        )
        tf = hdr_box.text_frame
        p = tf.paragraphs[0]
        p.text = g_title
        ...
        # Bullets Box
        b_box = s4.shapes.add_textbox(
            cx + Inches(0.2), cy + Inches(0.65), card_w - Inches(0.4), Inches(1.7)
        )
        tf = b_box.text_frame
        tf.word_wrap = True
    ...
    add_header(
        s5,
        "05",
        "ŞEFFAF SKORLAMA VE SERT VETO FİLTRELERİ",
        "Sinyaller kara kutu değildir; birleşik skor ve katı veto kuralları ile üretilir.",
    )

    # Top Card: Score Spectrum
    top_c = s5.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.5), Inches(11.733), Inches(2.0)
    )
    top_c.fill.solid()
    ...
    vetos = [
        (
            "⛔ 1. Yön ve Rejim Çelişkisi",
            "Trend ile Tetikleyici Uyumsuzluğu",
            "Ana trend yönü (örneğin düşen trend) ile anlık indikatör sinyali çelişiyorsa sinyal skoru otomatik olarak bastırılır veya veto edilir.",
            ACCENT_AMBER,
        ),
        (
            "⛔ 2. Aşırı Uzama (Chase Risk)",
            "Direnç Yakınlığı & Doygunluk",
            "Fiyat üst Bollinger bandına veya kritik dirence çok yakınsa chase (fiyatı kovalama) riski tespiti yapılır; skora ceza verilir.",
            ACCENT_RED,
        ),
        (
            "⛔ 3. Yapısal Zayıflık & Likidite",
            "ATR Uç Noktaları & Hacim Eksikliği",
            "Düşük işlem hacmi veya volatilite patlaması yaşanan ortamlarda sahte kırılmaları önlemek adına sinyal yürütmesi engellenir.",
            ACCENT_INDIGO,
        ),
    ]
    ...
    add_header(
        s6,
        "06",
        "OTONOM GERİ BESLEME VE MODEL KALİBRASYONU",
        "Zaman serisi hassasiyetini koruyan kontrollü makine öğrenmesi döngüsü.",
    )

    ml_steps = [
        (
            "01",
            "Sinyal Geçmişi & Özellik Kaydı",
            "Sinyal anındaki Skor, ADX, RSI, Hacim Katı, ATR, R/R Oranı ve Trend ölçekleri veritabanına eksiksiz kaydedilir.",
            ACCENT_CYAN,
        ),
        (
            "02",
            "İleri Dönem Etiketleme",
            "Üretilen sinyaller, N bar sonraki gerçekleşen getiri performansına göre Başarılı / Başarısız olarak etiketlenir.",
            ACCENT_INDIGO,
        ),
        (
            "03",
            "TimeSeriesSplit Eğitimi",
            "Gelecek bilgisinin sızmasını (Look-ahead bias) önlemek için TimeSeriesSplit ile XGBoost / Lojistik Regresyon eğitilir.",
            ACCENT_AMBER,
        ),
        (
            "04",
            "Canlı Olasılık Kalibrasyonu",
            "Ham model çıktısı Platt Scaling / Isotonic yöntemle kalibre edilir; üretimde güven olasılığı skoru üretir.",
            ACCENT_EMERALD,
        ),
    ]
    ...
    for i, (num_str, title_str, desc_str, clr_val) in enumerate(ml_steps):
        ...
        tf = s6.shapes.add_textbox(
            cx + Inches(0.15), cy + Inches(1.0), card_w - Inches(0.3), Inches(3.6)
        ).text_frame
        tf.word_wrap = True
        p = tf.paragraphs[0]
        p.text = title_str
    ...
    add_header(
        s7,
        "07",
        "WALK-FORWARD VE MALİYET STRESİ ALTINDA TEST SONUÇLARI",
        "Geriye dönük doğrulama sonuçları umut vericidir; risk sınırları şeffafça tanımlanmıştır.",
    )

    # Left Highlight Box 1
    hb1 = s7.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(1.6), Inches(5.6), Inches(2.3)
    )
    hb1.fill.solid()
    ...
    hb2 = s7.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(6.933), Inches(1.6), Inches(5.6), Inches(2.3)
    )
    hb2.fill.solid()
    ...
    rb = s7.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(4.2), Inches(11.733), Inches(2.7)
    )
    rb.fill.solid()
    ...
    r_items = [
        (
            "Tarihsel Veri Kısıtı:",
            " Elde edilen sonuçlar geçmiş piyasa rejimlerine ve kısıtlı işlem sayılarına dayanmaktadır; gelecekteki performansı doğrudan garanti etmez.",
        ),
        (
            "15 Dakika Veri Gecikmesi:",
            " Üretim ortamında 15 dk gecikmeli veri kullanımı ani volatilitede kayma (slippage) riskini artırır.",
        ),
        (
            "Survivorship Bias:",
            " Test evrenindeki hisseler güncel listeden seçildiği için geçmişte borsa kotundan çıkan hisselerin süzülmesi canlı ortamda dikkate alınmalıdır.",
        ),
    ]
    ...
    add_header(
        s8,
        "08",
        "MEVCUT ÇÖZÜMLERDEN FARKIMIZ",
        "Farkımız 'daha çok indikatör' değil; uçtan uca izlenebilir ve doğrulanabilir karar sistemidir.",
    )
    ...
    table_data = [
        (
            "Sinyal Üretimi",
            "Tek hisse / Tek grafik seviyesinde basit indikatör alarmı",
            "Tüm BIST evreninde çoklu zaman dilimi (MTF) & piyasa rejimi taraması",
        ),
        (
            "Açıklanabilirlik",
            "Sadece 'AL' ya da 'SAT' üreten şeffaf olmayan kara kutu",
            "Her sinyalin alt bileşen puanları, 4 kanıt grubu ve sert veto gerekçeleri",
        ),
        (
            "Risk & Yürütme",
            "Risk hesabı tamamen kullanıcının anlık insiyatifine bırakılır",
            "Dinamik Stop / Hedef seviyeleri, ATR pozisyon boyutu ve risk limitleri",
        ),
        (
            "Doğrulama Döngüsü",
            "Tekilleştirilmiş, maliyetsiz ve basit geriye dönük testler",
            "Walk-forward doğrulama, maliyet/kayma stresi ve otonom paper trade",
        ),
    ]
    ...
    add_header(
        s9,
        "09",
        "ÜRÜN VİZYONU VE YOL HARİTASI",
        "Araştırma prototipinden kurumsal ve güvenilir karar altyapısına geçiş planı.",
    )
    ...
    for i, (p_title, p_sub, p_bullets, accent_clr) in enumerate(phases):
        ...
        # Bullets Box
        b_box = s9.shapes.add_textbox(
            cx + Inches(0.15), cy + Inches(1.15), card_w - Inches(0.3), Inches(3.6)
        )
        tf = b_box.text_frame
        tf.word_wrap = True
    ...
    add_header(
        s10,
        "10",
        "MENTOR DEĞERLENDİRMESİ VE ODAK NOKTALARI",
        "Projenin bir sonraki aşamaya taşınmasında kritik tavsiye ve rehberlik alanları.",
    )

    questions = [
        (
            "1️⃣ Hedef Kullanıcı & Senaryo",
            "Bireysel yatırımcı paneli mi yoksa analist odaklı otonom karar destek motoru mu önceliklendirilmeli?",
            ACCENT_CYAN,
        ),
        (
            "2️⃣ Canlı Pilot Standartları",
            "Canlı emre geçiş öncesinde gerekli walk-forward kanıt eşiği ve risk sınırları ne olmalı?",
            ACCENT_AMBER,
        ),
        (
            "3️⃣ Veri & Entegrasyon Yol Haritası",
            "Lisanslı veri tedariki ve aracı kurum (broker) entegrasyonunda en verimli ve sürdürülebilir strateji hangisidir?",
            ACCENT_EMERALD,
        ),
    ]
    ...
    b_box = s10.shapes.add_shape(
        MSO_SHAPE.ROUNDED_RECTANGLE, Inches(0.8), Inches(5.4), Inches(11.733), Inches(1.4)
    )
    b_box.fill.solid()
